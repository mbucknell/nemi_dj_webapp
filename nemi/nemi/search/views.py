...
class BaseSearchView(View):
    ...
    '''Extends the standard view class to implement the search form portion of the search pages. The view
    only processes get requests. When a get request is received, the form is validated and then the query
    represented by the form is generated by calling the method get_query_and_context_from_form and put in qs. 
    Any additional information derived from the search form is put into the dictionary context. This class
    also provides methods which generate http responses for valid forms, invalid forms and empty forms.
    '''
    ...
    def get_valid_form_response(self, request, *args, **kwargs):
        '''Returns the http response when form validation succeeds'''
        return HttpResponse("Default response")
    
    def get_invalid_form_response(self, request, *args, **kwargs):
        '''Returns the http response when form validation fails.'''
        return HttpResponse("Invalid form")
    
    def get_empty_form_response(self, request, *args, **kwargs):
        '''Returns the http response when the form is not bound.'''
...
class MicrobiologicalSearchView(BaseSearchView, SearchResultMixin, TemplateResponseMixin):
    ...
    def get_query_and_context_from_form(self):
        self.context = {}
        criteria = []
        self.qs = MethodAnalyteAllVW.objects.all().filter(method_subcategory_id__exact=5)
        
        if self.search_form.cleaned_data['analyte'] != 'all' and self.search_form.cleaned_data['analyte'] != -9999:
