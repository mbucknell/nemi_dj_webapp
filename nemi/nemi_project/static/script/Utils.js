Utils = {
	...
	setSelect2Enabled: function (els /* jquery elements */, isEnabled /* Boolean */) {
		/* Set the enable state of els to isEnabled. Also set the state of the associate label element.
		 * This function assumes that the associated label has the same id as the select2 element with
		 * '-label' appended. 
		 */
		if (isEnabled) {
			els.select2('enable');
	...
	createSelectMenu : function (select2Id /* hidden element id */, url /* String */, data /* Object */ ){
		// Create a select2 option menu by retrieving the choices using url and data.
		$.ajax({
			url: url,
			data: data,
			success: function(resp) {
				var selectData = [];
				var maxChar = 0; // Used to set the width to the display value with the most characters.
				
				for (var i = 0; i < resp.choices.length; i++) {
					selectData.push({id: resp.choices[i].value, text: resp.choices[i].display_value});
