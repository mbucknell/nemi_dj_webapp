CustomHeaderDialog = {
    ...
    initialize : function(divEl, resultsTableEl) {
        ...
        resultsTableEl.find($('thead th')).not('.col-always-visible').each(
                function() {
                    var colName = $(this).find('.header-label').html();
                    var colIndex = $(this).index();
                    var colId = 'col-' + colIndex;
                    checkboxHtml += '<li><input type="checkbox" id="' + colId
                            + '" value="' + colIndex + '"';
                    if ($(this).is(':visible')) {
                        checkboxHtml += ' checked="checked"';
                    }
                    checkboxHtml += ' /><label for="' + colId + '">' + colName
                            + '</label></li>';
                });
                                            ...
                                            .each(
                                                    function() {
                                                        var colIndex = parseInt($(
                                                                this).val()) + 1;
                                                        var colEls = CustomHeaderDialog.tableEl
                                                                .find('tr :nth-child('
                                                                        + colIndex
                                                                        + ')');
                                                        if ($(this).is(
