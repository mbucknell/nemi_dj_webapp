...
from methods.utils.forms import get_criteria_from_field_data
...
class TestAddStatisticalSourceView(TestCase):
    ...
    def test_view_get(self):
        resp = self.client.get(reverse('methods-create_statistical_source')) 
        
        self.assertRedirects(resp, '/accounts/login/?next=%s' % (reverse('methods-create_statistical_source')))
...
class TestStatisticSearchViewNoMethods(TestCase):
    ...
    def test_view_no_query(self):
        resp = self.client.get(reverse('methods-statistics'))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_view_query_all(self):
        resp = self.client.get(reverse('methods-statistics'), {'item_type' : '', 
                                                              'complexity' : 'all', 
                                                              'analysis_types' : '',
...
class TestStatisticSearchView(TestCase):
    ...
    def test_filter_query(self):
        resp = self.client.get(reverse('methods-statistics'), {'item_type' : 3, 
                                                              'complexity' : 'all', 
                                                              'analysis_types' : '',
                                                              'publication_source_type' : '',
                                                              'design_objectives' : '',
